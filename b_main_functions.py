...
def calc_alpha(returns, risk_premium):
    ...
    Regression = smf.ols(formula='rate_of_returns ~ market_risk_premium', data=df_alpha) #Modell
...
def returns_weights(signals, original_returns, transitions, kind='normal', transaction_costs=0):
    ...
    if len(transitions['days']) > 1:
        df_weighted_returns['investments'] = abs(df_weighted_returns['weights'].replace(1, 0).shift() - \
                                                 df_weighted_returns['weights'].replace(1, 0)) #Wann wie viel prozentual ivestiert wird
    else:
        df_weighted_returns['investments'] = abs(df_weighted_returns['weights'].shift() - \
                                                 df_weighted_returns['weights']) #Falls alles am Folgetag umgesetzt werden soll   
    df_weighted_returns['investments'] = df_weighted_returns['investments'].fillna(0)
    df_weighted_returns['signal'] = signals #Signale
    df_weighted_returns['available'] = 1 - df_weighted_returns['weights'] #Verfügbares Kapital
    ...
    for index, row in df_weighted_returns_copy.iterrows():
        if counter == 1: 
            ...
            if row['investments'] != 0:
                previous_value = (previous_value * row['investments']) * (1 + row['original_returns']*row['signal']) + \
                    (previous_value * (1 - row['investments']))#Wert des investierten Kapitals
            else: 
                previous_value = (row['signal'] * row['original_returns'] + 1) * previous_value
            if np.isnan(previous_value) == True:
                previous_value = 0
        df_weighted_returns.loc[index, 'investment_gains'] = previous_value    
    df_weighted_returns['return_transition'] = rate_of_return(df_weighted_returns['investment_gains'], kind='normal') #Rendite
    df_weighted_returns['value_when_invested'] = df_weighted_returns['investment_gains'].copy()
    df_weighted_returns['value_when_invested'][df_weighted_returns['investments']==0] = 0
    df_weighted_returns['transaction_costs'] = df_weighted_returns['value_when_invested'] * transaction_costs #Transaktionskosten
    df_weighted_returns['return_transition_with_costs'] = df_weighted_returns['return_transition'] - df_weighted_returns['transaction_costs']
    return df_weighted_returns['return_transition'].replace(math.inf, 0), df_weighted_returns['return_transition_with_costs'].replace(math.inf, 0) 
...
def crossover_signal(series, avg_short=38, avg_long=200):
    ...
    try:
        ...
        momentum_df['avg_short'] = momentum_df.iloc[:,0].rolling(avg_short, win_type=None).mean()
        momentum_df['avg_long'] = momentum_df.iloc[:,0].rolling(avg_long, win_type=None).mean()        
...
def average_true_range(high_series, low_series, close_series, true_range=20):
    ...
    dataframe_atr['AverageTrueRange'] = dataframe_atr['TrueRange'].rolling(true_range).mean()
...
def clenow_counter_plunger_criteria(high_series, low_series, close_series, 
                                    trend='ma', mean_lower=38, mean_higher=200, window_reading=20, 
                                    true_range=20, plunge_trigger=3):
    ...
    if trend == 'ewm':
        dataframe_ccp['trend'] = dataframe_ccp['px_last'].ewm(span=mean_lower, min_periods=mean_lower).mean() - \
        dataframe_ccp['px_last'].ewm(span=mean_higher, min_periods=mean_higher).mean() 
        dataframe_ccp['trend'] = dataframe_ccp['trend'].apply(calc_trend) #Trend EWM
    elif trend == 'ma':
        dataframe_ccp['trend'] = dataframe_ccp['px_last'].fillna(method='bfill').rolling(mean_lower).mean() - \
        dataframe_ccp['px_last'].fillna(method='bfill').rolling(mean_higher).mean()
        dataframe_ccp['trend'] = dataframe_ccp['trend'].apply(momentum)
        dataframe_ccp['trend'] = dataframe_ccp['trend'].fillna(0)
    dataframe_ccp['atr'] = average_true_range(high_series, low_series, close_series, true_range=true_range) #ATR
    dataframe_ccp['reading_max'] = high_series.fillna(method='bfill').rolling(window=window_reading).max()
    dataframe_ccp['reading_min'] = low_series.fillna(method='bfill').rolling(window=window_reading).min()
    dataframe_ccp['plunge_up'] = (dataframe_ccp['reading_max'] - close_series) / dataframe_ccp['atr']
    dataframe_ccp['plunge_down'] = (close_series - dataframe_ccp['reading_min']) / dataframe_ccp['atr']
...
def plunger_signal(df_plunger, confirmation_period=2):
    ...
    for index, row in df_plunger_signal.iterrows():
        ...
        #Bereits Long
            ...
            #Kein Exit
            if ((row['trend'] == 1) & (row['px_last'] < target) & (row['px_last'] > stop)):
                df_plunger_signal.loc[index, 'signal'] = 1
