...
class signals_analysis:
    ...
    #% Methodische Einstellungen
        ...
        self.transitions = {'days': [1, 2, 3],
                            'portions': [0.33, 0.67, 1]}
        #% Bootstraps
        ...
        self.confirmation_period = 2 #Confirmation Period (lediglich bezüglich des Exits!)
        #% Z-Scores
    ...
    #% Einlesen der DataFrames und Series
        ...
        df_market = pd.read_csv(self.filename, header=0, index_col='date', parse_dates=['date'], dayfirst=True)
        os.chdir(self.wd_other_data)
        risk_premium_rough = pd.read_csv(self.risk_premiums_file, header=0, index_col='date', parse_dates=['date'], dayfirst=True)
        risk_premium = df_market.merge(risk_premium_rough, how='left', on='date')[self.risk_premium_column]
    
    #% Benchmark ohne Bootstraps
        bench_wo_boot = {}
        bench_return_rates = {}
        #% Buy-and-Hold
        buy_and_hold_returns = mf.rate_of_return(df_market[self.close_column], kind=self.return_kind)
        ...
        bench_wo_boot['buy_and_hold'] = [buy_and_hold_sr, buy_and_hold_alpha,
                                          buy_and_hold_std, buy_and_hold_mean, buy_and_hold_gmean, 
                                          buy_and_hold_median, buy_and_hold_25p, buy_and_hold_75p]
        
        #% Crossover gemäß Transition
        signals = df_market[self.crossover_column]
        co_wo_cost_returns, co_w_cost_returns = mf.returns_weights(signals, buy_and_hold_returns, self.transitions, 
        ...
        bench_wo_boot['crossover_wo_costs'] = [co_wo_cost_sr, co_wo_cost_alpha,
                                                co_wo_cost_std, co_wo_cost_mean, co_wo_cost_gmean, co_wo_cost_median,
                                                co_wo_cost_25p, co_wo_cost_75p]
        ...
        bench_wo_boot['crossover_w_costs'] = [co_w_cost_sr, co_w_cost_alpha,
                                                co_w_cost_std, co_w_cost_mean, co_w_cost_gmean, co_w_cost_median,
                                                co_w_cost_25p, co_w_cost_75p]
        
        #% Transition Countertrend
        signals = df_market[self.countertrend_column]
        ct_wo_cost_returns, ct_w_cost_returns = mf.returns_weights(signals, buy_and_hold_returns, self.transitions, 
                                                                    kind=self.return_kind, 
                                                                    transaction_costs=self.transaction_costs)
        ...
        bench_wo_boot['countertrend_wo_costs'] = [ct_wo_cost_sr, ct_wo_cost_alpha,
                                                  ct_wo_cost_std, ct_wo_cost_mean, ct_wo_cost_gmean, 
                                                  ct_wo_cost_median, ct_wo_cost_25p, ct_wo_cost_75p]
        ...
        bench_wo_boot['countertrend_w_costs'] = [ct_w_cost_sr, ct_w_cost_alpha,
                                                  ct_w_cost_std, ct_w_cost_mean, ct_w_cost_gmean,
                                                  ct_w_cost_median, ct_w_cost_25p, ct_w_cost_75p]
        #% Transition z-scores
        signals = mf.z_score_signals(df_market[self.close_column], window=self.zscore_window, period=self.zscore_period)
        zs_wo_cost_returns, zs_w_cost_returns = mf.returns_weights(signals, buy_and_hold_returns, self.transitions, 
                                                                   kind=self.return_kind, 
                                                                   transaction_costs=self.transaction_costs)
        ...
        bench_wo_boot['zscore_wo_costs'] = [zs_wo_cost_sr, zs_wo_cost_alpha,
                                            zs_wo_cost_std, zs_wo_cost_mean, zs_wo_cost_gmean, zs_wo_cost_median,
                                            zs_wo_cost_25p, zs_wo_cost_75p]
        ...
        bench_wo_boot['zscore_w_costs'] = [zs_w_cost_sr, zs_w_cost_alpha,
                                            zs_w_cost_std, zs_w_cost_mean, zs_w_cost_gmean, zs_w_cost_median,
                                            zs_w_cost_25p, zs_w_cost_75p]
        
        #% Zusammenführen der Ergebnisse
        df_bench = pd.DataFrame(bench_wo_boot)
        df_bench.index = ['sharpe_ratio', 'alpha', 'std', 'mean', 'gmean', 'median', '25p', '75p']
        ...
        returns_filename = self.filename[:3] + '_return_rates_benchmark_' + self.export_kind + '.csv'
    ...
    def main_bootstraps(self):
        ...
        if self.bootstraps_replicates == 0:
            ...
            df_market = pd.read_csv(self.filename, header=0, index_col='date', parse_dates=['date'], dayfirst=True)
            os.chdir(self.wd_other_data)
            risk_premium_rough = pd.read_csv(self.risk_premiums_file, header=0, 
                                             index_col='date', parse_dates=['date'], dayfirst=True)
            risk_premium = df_market.merge(risk_premium_rough, how='left', on='date')[self.risk_premium_column]
            
            #% Ursprungsrendite
            original_returns = mf.rate_of_return(df_market[self.close_column], kind=self.return_kind)
            
            #% Bootstraps
            bootstraps = mf.bootstrap(df_market[self.close_column], original_returns, 
                                      number_replicates=self.bootstraps_replicates, 
                                      chunksize=self.chunksize, seed=self.seed)
            ...
            for bootstrap in bootstraps:
                ...
                buy_and_hold_returns = mf.rate_of_return(bootstrap_series, kind=self.return_kind)
                ...
                #% Crossover gemäß Transition
                crossover_signal = mf.crossover_signal(bootstrap_series, avg_short=self.co_avg_short, 
                                                       avg_long=self.co_avg_long)
                co_wo_cost_returns, co_w_cost_returns = mf.returns_weights(crossover_signal, buy_and_hold_returns, 
                                                                           self.transitions, kind=self.return_kind, 
                                                                           transaction_costs=self.transaction_costs)
                    #% Ohne Transaktionskosten
                ...
                rolling_std = close_series.rolling(self.std_window).std().fillna(method='bfill')
                high_series = close_series + rolling_std
                low_series = close_series - rolling_std
                plunger_criteria_df = mf.clenow_counter_plunger_criteria(high_series, low_series, close_series, 
                                                                         trend=self.trend, 
                ...
                plunger_signal = mf.plunger_signal(plunger_criteria_df, confirmation_period=self.confirmation_period)
                ct_wo_cost_returns, ct_w_cost_returns = mf.returns_weights(plunger_signal, buy_and_hold_returns, 
                                                                           self.transitions, kind=self.return_kind, 
                                                                           transaction_costs=self.transaction_costs)
                    #% Ohne Transaktionskosten
                ...
                zscore_signal = mf.z_score_signals(bootstrap_series, window=self.zscore_window, period=self.zscore_period)
                zs_wo_cost_returns, zs_w_cost_returns = mf.returns_weights(zscore_signal, buy_and_hold_returns, 
                                                                           self.transitions, kind=self.return_kind, 
                                                                           transaction_costs=self.transaction_costs)
                    #% Ohne Transaktionskosten
            ...
            bootstraps_results_dict = {'buy_and_hold_sr': buy_and_hold_sr, 'buy_and_hold_alpha': buy_and_hold_alpha,
                                       'buy_and_hold_std': buy_and_hold_std, 'buy_and_hold_mean': buy_and_hold_mean,
                                       'buy_and_hold_gmean': buy_and_hold_gmean,
                                       'buy_and_hold_median': buy_and_hold_median, 'buy_and_hold_25p': buy_and_hold_25p,
                                       'buy_and_hold_75p': buy_and_hold_75p,
                                       
                                       'co_wo_cost_sr': co_wo_cost_sr, 'co_wo_cost_alpha': co_wo_cost_alpha,
                                       'co_wo_cost_std': co_wo_cost_std, 'co_wo_cost_mean': co_wo_cost_mean,
                                       'co_wo_cost_gmean': co_wo_cost_gmean,
                                       'co_wo_cost_median': co_wo_cost_median, 'co_wo_cost_25p': co_wo_cost_25p,
                                       'co_wo_cost_75p': co_wo_cost_75p,
                                       
                                       'co_w_cost_sr': co_w_cost_sr, 'co_w_cost_alpha': co_w_cost_alpha,
                                       'co_w_cost_std': co_w_cost_std, 'co_w_cost_mean': co_w_cost_mean,
                                       'co_w_cost_gmean': co_w_cost_gmean,
                                       'co_w_cost_median': co_w_cost_median, 'co_w_cost_25p': co_w_cost_25p,
                                       'co_w_cost_75p': co_w_cost_75p,
                                       
                                       'ct_wo_cost_sr': ct_wo_cost_sr, 'ct_wo_cost_alpha': ct_wo_cost_alpha,
                                       'ct_wo_cost_std': ct_wo_cost_std, 'ct_wo_cost_mean': ct_wo_cost_mean,
                                       'ct_wo_cost_gmean': ct_wo_cost_gmean,
                                       'ct_wo_cost_median': ct_wo_cost_median, 'ct_wo_cost_25p': ct_wo_cost_25p,
                                       'ct_wo_cost_75p': ct_wo_cost_75p,
                                       
                                       'ct_w_cost_sr': ct_w_cost_sr, 'ct_w_cost_alpha': ct_w_cost_alpha,
                                       'ct_w_cost_std': ct_w_cost_std, 'ct_w_cost_mean': ct_w_cost_mean,
                                       'ct_w_cost_gmean': ct_w_cost_gmean,
                                       'ct_w_cost_median': ct_w_cost_median, 'ct_w_cost_25p': ct_w_cost_25p,
                                       'ct_w_cost_75p': ct_w_cost_75p,
                                       
                                       'zs_wo_cost_sr': zs_wo_cost_sr, 'zs_wo_cost_alpha': co_wo_cost_alpha,
                                       'zs_wo_cost_std': zs_wo_cost_std, 'zs_wo_cost_mean': co_wo_cost_mean,
                                       'zs_wo_cost_gmean': zs_wo_cost_gmean,
                                       'zs_wo_cost_median': zs_wo_cost_median, 'zs_wo_cost_25p': co_wo_cost_25p,
                                       'zs_wo_cost_75p': zs_wo_cost_75p,
                                       
                                       'zs_w_cost_sr': zs_w_cost_sr, 'zs_w_cost_alpha': zs_w_cost_alpha,
                                       'zs_w_cost_std': zs_w_cost_std, 'zs_w_cost_mean': zs_w_cost_mean,
                                       'zs_w_cost_gmean': zs_w_cost_gmean,
                                       'zs_w_cost_median': zs_w_cost_median, 'zs_w_cost_25p': zs_w_cost_25p,
                                       'zs_w_cost_75p': zs_w_cost_75p}
            
            bootstraps_results = pd.DataFrame(bootstraps_results_dict)
            #% Abspeichern der Ergebnisse
            os.chdir(self.wd_export)
            export_filename = self.filename[:3] + '_bootstraps_' + self.export_kind + '.csv'
