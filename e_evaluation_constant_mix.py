...
df_results['date'] = df_results['date'].apply(lambda x: pd.to_datetime(x, dayfirst=True))
...
df_risk_premium = pd.read_csv('Risk_Premium.csv', header=0, index_col= 'date', parse_dates=True, dayfirst=True)
...
for column in df_results.columns: 
    ...
    if column_elements[0] == 'full':
        ...
        if column_elements[-1] == 'return':
            mean_t_value, mean_p_value = stats.ttest_ind(df_results['full_total_return'], df_results[column])
        else:
            mean_t_value, mean_p_value = stats.ttest_ind(df_results['full_total_return-tk'], df_results[column])
    
        #% Zusammenführen der Ergebnisse
    results_li = [sr, alpha, std, mean, g_mean, median, p25, p75, mean_t_value, mean_p_value]
    results_dict[column] = results_li
    
df_results_hypothesistest = pd.DataFrame(results_dict)

df_results_hypothesistest.index = ['sr', 'alpha', 'std', 'mean', 'g_mean', 'median', 'p25', 'p75', 
                                   'mean_t_value', 'mean_p_value']
