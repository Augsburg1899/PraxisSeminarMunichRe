...
for market in df_benchmark_files['market'].unique():
    for transition in df_benchmark_files['transition'].unique():
        df_selected = df_benchmark_files[(df_benchmark_files['market'] == market) & \
        ...
        for column in df_selected.columns:
            if transition == 'full_transition':
                ...
                bench_df = df_benchmark_files[(df_benchmark_files['market'] == market) & \
                                         (df_benchmark_files['transition'] == 'full_transition')]
                t_value, p_value = stats.ttest_ind(bench_df[column], df_selected[column])
                li_selected = [np.mean(df_selected[column]), t_value, p_value]
                name = market + '_' + transition + '_' + column
                stats_bench_dict[name] = li_selected
...
for market in df_bootstrap_files['market'].unique():
    for transition in df_bootstrap_files['transition'].unique():
        df_market_transition = df_bootstrap_files[(df_bootstrap_files['market'] == market)& \
...
for market in df_bootstrap_files['market'].unique():
    for transition in df_bootstrap_files['transition'].unique():
        if transition == 'full_transition':
            ...
            df_benchmark = df_bootstrap_files[(df_bootstrap_files['market'] == market)& \
                                                      (df_bootstrap_files['transition'] == 'full_transition')]
            df_benchmark = df_benchmark.iloc[:,:56]
            stats_li = []
            for column in df_market_transition.columns:
                t_value, p_value = stats.ttest_ind(df_benchmark[column], df_market_transition[column])
                if t_value > 0:
                    if p_value <= 0.01:
                        value = '*** (+)'
...
df_stats_with_significance_basispunkte = df_stats_with_significance_basispunkte.astype(str) + significance_df.fillna('')
df_stats_with_significance_basispunkte.to_csv('df_stats_with_significance_basispunkte.csv', header=True)


#Hypothesentest über alle Bootstraps hinweg
df_hypothesis_test = df_bootstrap_files[df_bootstrap_files['transition']=='full_transition'].mean()
df_hypothesis_test = pd.DataFrame(df_hypothesis_test)
df_hypothesis_test.columns = ['full_transition']
df_hypothesis_test['linear_2d'] = df_bootstrap_files[df_bootstrap_files['transition']=='linear_2d'].mean()
df_hypothesis_test['linear_3d'] = df_bootstrap_files[df_bootstrap_files['transition']=='linear_3d'].mean()
df_hypothesis_test['progressive_2d'] = df_bootstrap_files[df_bootstrap_files['transition']=='progressive_2d'].mean()
df_hypothesis_test['progressive_3d'] = df_bootstrap_files[df_bootstrap_files['transition']=='progressive_3d'].mean()
...
def hypothesis_test(column_name, transition):
    ...
    if column_name_elements[0] == 'buy':
        ...
        a = df_bootstrap_files[df_bootstrap_files['transition']==transition][column_name]
        b = df_bootstrap_files[df_bootstrap_files['transition']=='full_transition'][column_name]
...
for transition in ['linear_2d', 'linear_3d', 'progressive_2d', 'progressive_3d']:
    ...
    df_hypothesis_test[column_name] = [hypothesis_test(column, transition) for column in df_hypothesis_test.index]
...
for transition in ['linear_2d', 'linear_3d', 'progressive_2d', 'progressive_3d']:
    ...
    df_hypothesis_test[significance_column] = df_hypothesis_test[significance_column].apply(significance)
