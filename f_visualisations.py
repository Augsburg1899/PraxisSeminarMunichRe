...
import a_exploratory_data_analysis as eda
...
os.chdir(wd_data)
df_us1 = pd.read_csv('us1_index.csv', sep=',', header=0, parse_dates=['date'], dayfirst=True).set_index('date')
df_es1 = pd.read_csv('es1_index.csv', sep=',', header=0, parse_dates=['date'], dayfirst=True).set_index('date')

    #% Erstellen der Bootstraps
bootstraps_us1 = mf.bootstrap(series=df_us1['px_last'], return_rates=mf.rate_of_return(df_us1['px_last']), 
                              number_replicates=10, chunksize=180, seed=123)
bootstraps_es1 = mf.bootstrap(series=df_es1['px_last'], return_rates=mf.rate_of_return(df_es1['px_last']), 
                              number_replicates=10, chunksize=180, seed=123)

    #% Plotten der Bootstraps
mf.plot_bootstraps(series=df_us1['px_last'], bootstraps=bootstraps_us1, 
                title='30 Year U.S. T-Bond Future', market_label='US1')
mf.plot_bootstraps(series=df_es1['px_last'], bootstraps=bootstraps_es1, 
                title='E-Mini S&P 500 Future', market_label='ES1')
...
for file in glob.glob('*.csv'):
    df = pd.read_csv(file, sep=',', header=0, parse_dates=['date'], dayfirst=True).set_index('date')
...
_ = plt.title('Boxplots der Tagesrenditen aller Märkte', pad=20, fontweight='bold', size='xx-large')
...
fig = plt.figure(figsize=(50,30))
ax = plt.axes(frameon=False)
_ = sns.lineplot(x='date', y='px_last', hue='market', data=df_closing_prices, palette='GnBu')
...
df_crossover_us1['Signal'] = pd.Categorical(df_us1['crossover_signal'].replace(1, 'Long')\
                                            .replace(0, 'Kein Signal').replace(-1, 'Short'))
...
ax = plt.axes(frameon=False)
_ = sns.scatterplot(x=df_crossover_us1.index, y='Kurswert', hue='Signal', palette='viridis', 
                 data=df_crossover_us1, marker='x')
_ = plt.legend(loc='upper right', edgecolor='white')
_ = plt.ylabel('Kurswert', fontweight='bold')
_ = plt.xlabel('Zeit', fontweight='bold')
_ = plt.title('30 Year U.S. T-Bond Future - Crossover Strategie', pad=20, fontweight='bold')
plt.show()
plt.close()

    #% Countertrend
        #% Zusammenführen der Daten
df_crossover_us1['Signal'] = pd.Categorical(df_us1['countertrend_signal'].replace(1, 'Long')\
                                            .replace(0, 'Kein Signal').replace(-1, 'Short'))
...
_ = plt.ylabel('Kurswert', fontweight='bold')
_ = plt.xlabel('Zeit', fontweight='bold')
_ = plt.title('30 Year U.S. T-Bond Future - Countertrend Strategie', pad=20, fontweight='bold')
plt.show()
plt.close()
